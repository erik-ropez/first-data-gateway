...
/**
 * DecryptedGooglePay Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://openapi-generator.tech
 */
class DecryptedGooglePay implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'DecryptedGooglePay';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['cryptogram']) && !preg_match("/^(?!\\s*$).+/", $this->container['cryptogram'])) {
    ...
    public function setAccountNumber($accountNumber)
    {
        ...
        if ((!preg_match("/[0-9]{13,19}/", $accountNumber))) {
            throw new \InvalidArgumentException("invalid value for $accountNumber when calling DecryptedGooglePay., must conform to the pattern /[0-9]{13,19}/.");
        }
    ...
    public function setExpiration($expiration)
    {
        ...
        if ((!preg_match("/[0-9]{6}/", $expiration))) {
            throw new \InvalidArgumentException("invalid value for $expiration when calling DecryptedGooglePay., must conform to the pattern /[0-9]{6}/.");
        }
    ...
    public function setCardholderName($cardholderName)
    {
        if (!is_null($cardholderName) && (mb_strlen($cardholderName) > 96)) {
            throw new \InvalidArgumentException('invalid length for $cardholderName when calling DecryptedGooglePay., must be smaller than or equal to 96.');
        }
        if (!is_null($cardholderName) && (!preg_match("/^(?!\\s*$).+/", $cardholderName))) {
            throw new \InvalidArgumentException("invalid value for $cardholderName when calling DecryptedGooglePay., must conform to the pattern /^(?!\\s*$).+/.");
        }
    ...
    public function setBrand($brand)
    {
        ...
        if (!is_null($brand) && (!preg_match("/^(?!\\s*$).+/", $brand))) {
            throw new \InvalidArgumentException("invalid value for $brand when calling DecryptedGooglePay., must conform to the pattern /^(?!\\s*$).+/.");
        }
     ...
     * Sets cryptogram
     *
     * @param string|null $cryptogram The wallet cryptogram from the decrypted data.
     *
     * @return $this
     */
    public function setCryptogram($cryptogram)
    {
        ...
        if (!is_null($cryptogram) && (!preg_match("/^(?!\\s*$).+/", $cryptogram))) {
            throw new \InvalidArgumentException("invalid value for $cryptogram when calling DecryptedGooglePay., must conform to the pattern /^(?!\\s*$).+/.");
        }
    ...
    public function setEciIndicator($eciIndicator)
    {
        ...
        if (!is_null($eciIndicator) && (!preg_match("/[0-9]{2}/", $eciIndicator))) {
            throw new \InvalidArgumentException("invalid value for $eciIndicator when calling DecryptedGooglePay., must conform to the pattern /[0-9]{2}/.");
        }
